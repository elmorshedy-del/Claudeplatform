    ...
    try {
      ...
      if (settings.deployMode === 'safe' && !currentBranch) {
        ...
        setMessages(prev => [...prev, {
          id: `branch-${Date.now()}`,
          role: 'assistant',
          content: `📦 Created branch \`${branchName}\` for safe editing. Your main branch is protected.`,
          timestamp: new Date(),
        }]);
      }
  ...
  const handleMerge = async () => {
    ...
    try {
      ...
      setMessages(prev => [...prev, {
        id: `merge-${Date.now()}`,
        role: 'assistant',
        content: `✅ Merged to main and cleaned up branch. Your changes are now live!`,
        timestamp: new Date(),
      }]);
  ...
  const handleDiscard = async () => {
    ...
    try {
      ...
      setMessages(prev => [...prev, {
        id: `discard-${Date.now()}`,
        role: 'assistant',
        content: `🗑️ Discarded branch \`${currentBranch}\`. Main branch unchanged.`,
        timestamp: new Date(),
      }]);
      ...
      setMessages(prev => [...prev, {
        id: `error-${Date.now()}`,
        role: 'assistant',
        content: `❌ Failed to discard: ${error.message}`,
        timestamp: new Date(),
      }]);
    } finally {
      setIsLoading(false);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-4">
        <div className="w-full max-w-md bg-dark-800 rounded-xl border border-dark-600 p-8">
          <h1 className="text-2xl font-bold mb-2 text-center">🚀 Claude Coder</h1>
              ...
              >
                ...
                {message.cost !== undefined && (
                  <div className="mt-2 text-xs text-gray-400">
                    ${message.cost.toFixed(4)} • {message.tokensUsed?.input || 0} in / {message.tokensUsed?.output || 0} out
